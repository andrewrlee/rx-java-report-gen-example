...
public class TestDataSource {
    private static final Logger L = LoggerFactory.getLogger(TestDataSource.class);
    private static final AtomicInteger TOTAL_ROW_COUNT = new AtomicInteger(0);
    private static int FAILS_ON = -1;

    private static final String[] FIELDS = new String[] {"name", "age", "phone", "address-line-1", "address-line-2", "postcode"};
    ...
    private String[] getLine() {
        ...
        if (thisRow == FAILS_ON) {
            throw new RuntimeException("An Error occured! in source: " + name + ", after: " + rowNumber + " rows");
        }
        return stream(FIELDS).map(field -> String.format("%s-%s-%s", field, name, rowNumber)).toArray(String[]::new);
    }
